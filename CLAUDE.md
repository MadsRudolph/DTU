...
- **34870 Lab B — PREPARED, slot Tue 22-Sep-2026 10:00–12:00** (b.354 anechoic room 028 + control room 025; Group 10 = Louis Andrianne, Sophie Kimura, Mads; Lab B+C quiz individual, deadline **Mon 5-Oct**). Other Group 10 slots from the lab schedule sheet: **Lab C Tue 29-Sep 10:30 r.026, Lab D Tue 6-Oct 08:00 r.026, Lab E Tue 20-Oct 10:00 r.028/025**, loudspeaker **System D** shared with Group 4 (all four overlap the 34840 Tuesday lecture). The staff re-uploaded the measurement files on 21-Sep (`34870 - Lab B (2).zip`): only the sound-card detection changed (`Line In` / `Speakers` on the internal card instead of the USB card); `matlab/course/` holds the new version and `matlab/labB_devices.m` prints which devices will be picked, run it first on the lab PC. `process_labB` uses the newest repeat of every tag and plots the end-reference drift; both verified in a MATLAB dry run with a fake sound card on 22-Sep. Lab-day log template: `Lab B/data/labB_log.md`. In the labs repo `Labs/Lab B/`: `bem/run_bem.m` + `bem_results.mat/.csv` (course BEM `CylinderPlaneWave` for 0–180°, round and flat end; **MATLAB R2026a runs headless here**: `matlab -batch run_bem`, ~13 s per angle, always redirect stdin from /dev/null; the 106 MB package is unpacked to `bem/package/`, gitignored, from `BEM_FreeField.zip` in the vault `Labs/`), `matlab/measure_labB.m` (lab-day wrapper: brief's parameters, saves `data/labB_<tag>.mat`, shows signal vs noise) and `matlab/process_labB.m` (normalise by the no-mock-up reference, BEM overlay, scaling to 1", 1/2", 1/4", 1/8", optional 1/r plot; tested end to end on synthetic data). Prediction: 0 dB below ka ≈ 0.3, **+9.9 dB at 1345 Hz (ka = 3.1) at 0°**, ≈ +2 dB at 90°; ka = 1 at 437 Hz. Vault note `Labs/Lab B - Preparation.md` (procedure, checklist, expected curves, likely quiz questions), site section `#labB` with a bench driven by the BEM data. **Next:** after the lab slot put the `.mat` files in `Lab B/data/`, set the measured diameter in `process_labB.m`, run it, write the runthrough.
