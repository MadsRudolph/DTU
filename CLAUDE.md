...
- **The Analogy Bench (14-Sep-2026)** — interactive study site for 34870 lectures 0–5 **with every problem set and official solution** (`problems.js`), **and Lab A** (four live benches in `labs.js`, one per lab part): `5. Semester/Electroacoustics/Analogy Bench/` (`src/`, `python3 build.py`, `./deploy.sh`). **Public at https://study.madsrudolph.dev** (Cloudflare static-assets Worker, `npm run deploy:cf`; Wrangler is logged in via `~/.config/.wrangler`), mirrored on the home server **CT 116 `study`, http://192.168.50.220** (nginx, `./deploy.sh`), and as a Claude artifact (URL in the README). Extend it lecture by lecture: one `bench*()` per idea in `benches.js`, quiz questions in `quiz.js`.
