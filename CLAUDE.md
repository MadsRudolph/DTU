...
- **34870 Lab A (quiz deadline Sun 20-Sep)** — fully worked, 10-Sep-2026. LaTeX report `5. Semester/Electroacoustics/Labs/Lab A/report/LabA_report.tex` (PDF carried by Syncthing, copy in the vault `Labs/`), Obsidian runthrough `Obsidian/Courses/34870 Electroacoustics/Labs/Lab A - Runthrough.md` (quiz-ready answers per sub-question + LTspice translation), KiCad/ngspice projects under `Labs/Lab A/KiCad/PartN_*` regenerated by `Labs/Lab A/sim/partN.py`. **LTspice (20-Sep):** ready-to-run, headless-verified schematics for all four parts in `Labs/Lab A/LTspice/` (`gen_ltspice.py --verify` runs LTspice 26 in batch under wine and checks 33 numbers). That rebuild exposed an error in the first runthrough: the silencer's volume-velocity peaks are **+38.6/+41.0/+41.4 dB at 47/176/191 Hz**, not +26 dB — a 100 MΩ DC-leak resistor across the current source had loaded the ladder at its anti-resonances (|Z_in| up to 1.5 GΩ), and 200 points/decade clipped the high-Q peaks. Runthrough, site and KiCad generator are corrected; **the LaTeX report PDF still carries the old part-2 numbers** (rerun `sim/part2.py`, patch `LabA_report.tex`, rebuild with tectonic if the report is ever handed in). Remaining by hand: take the screenshots and submit the quiz in DTU Learn.
