...
- **The Analogy Bench (14-Sep-2026)** — interactive study site for 34870 lectures 0–5: `5. Semester/Electroacoustics/Analogy Bench/` (`src/`, `python3 build.py`, `./deploy.sh`). Hosted on the home server **CT 116 `study`, http://192.168.50.220** (nginx, LAN only — a public `*.madsrudolph.dev` name needs a one-time `cloudflared tunnel login`; no Cloudflare credentials exist on the PCs). Also published as a Claude artifact (URL in the README). Extend it lecture by lecture: one `bench*()` per idea in `benches.js`, quiz questions in `quiz.js`.
