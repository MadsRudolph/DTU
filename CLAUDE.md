...
- **34870 MATLAB Live Scripts for every problem set (24-Sep-2026)** — `5. Semester/Electroacoustics/Matlab/` (index `README.md`): Lectures 1, 2 (+ bonus), 3, 4A, Problems 4, 5, 6, 7, each `.m` source + `.mlx` (gitignored, Syncthing) + `.html` export. Style Mads asked for: formula stated first as an equation, then one plain MATLAB line, result shown, official answer next to it; no helper functions/loops/printf. Matching LTspice generators with `--verify`/`--preview` in `5. Semester/Electroacoustics/LTspice/<Lecture N | Problems N>/` (all reuse the Lab A builder). Headless `export` needs `CatchError=true` on this install. Known official-solution discrepancies are listed in the Matlab README (e.g. Problems 6 M250 9.6 comes from a mistyped 1.1m gain).
- **34870 Lecture 7 (moving-coil loudspeakers, 24-Sep)** — note `Lecture Notes/Lecture 7 - Moving Coil Loudspeakers.md` written from the deck + Problems 7 (no solutions or recording yet; every bracket answer reproduced). Problem 4 in LTspice: `5. Semester/Electroacoustics/LTspice/Problems 7 - Loudspeaker/p7.py` (prints P1–3, writes `P7_315SWR_Baffle{,_LossyLe}.asc`, `--verify --plots` runs LTspice headless against the closed form and writes `Images/Lecture7/`; reuses the Lab A builder from the labs repo). Gotcha: the data sheet's M_MS includes the air, so the circuit's mechanical mass is M_MD = 88.2 − 2·S_D²·M_A1 = 74.6 g. Site section `#l7` with a driver bench and a Lab D T-S-measurement bench.
...
- **The Study Bench (14-Sep-2026)** — `5. Semester/Study Bench/` (`core/`, `courses/<code>/`, `hub/`, `python3 build.py`, `./deploy.sh`): hub at study.madsrudolph.dev with **34870 *The Analogy Bench*** (lectures 0–7, every problem set with official solutions, Lab A with four live benches, Lab B with a BEM-data bench, Lab C with an f_s/Q → backplate bench) and **34840 *The Sound Bench*** (lectures 1–3, 12 benches, weeks 1–3 problems; lecture 3 written from the slides ahead of the 15-Sep lecture, check it against the notes afterwards). **Public at https://study.madsrudolph.dev** (Cloudflare static-assets Worker, `npm run deploy:cf`; Wrangler is logged in on **both** PCs via `~/.config/.wrangler` since 24-Sep-2026), mirrored on the home server **CT 116 `study`, http://192.168.50.220** (nginx, `./deploy.sh`), and as a Claude artifact (URL in the README). Extend it lecture by lecture per the `study-bench` skill.
- **KiCad ↔ ngspice recipe** (controlled sources): instance fields `Sim.Device=SPICE`, `Sim.Params=type=\"G\" model=\"<gain>\"` with escaped quotes; G-source current flows N+ → N−; every node needs a DC path (100 MΩ leak). Written up in `Labs/Lab A/README.md`. LaTeX on **both** machines: `~/.local/bin/tectonic` 0.17.0 (no TeX Live installed; the laptop got the GitHub release binary on 24-Sep-2026).
