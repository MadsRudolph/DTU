...
s = tf('s');
G = 0.1*(s + 2)*(s + 100)/((s + 0.1)*(s^2 + 11.35*s + 25.59)); % To behave like 2nd order LPF
% (s + 2)/((s + 0.1)*(s^2 + 11.35*s + 25.59))
w = linspace(1e-4,254,5000);        % Define frequency range for Bode plot
...
zeta = overshoot2damping(M_p);
% Calculate the natural frequency
omega_n = 4/(t_s*zeta);
% Calculate phase margin
gamma_M = overshoot2phase_margin(M_p);
% Calculate the crossover frequency
omega_i = w_out(find(M == max(M),1,'first'));
omega_c = crossover_second_order(omega_n,zeta); % From settling time t_s
% omega_n = 1.8/t_r;
% omega_c = crossover_second_order(omega_n,zeta); % From rising time t_r
