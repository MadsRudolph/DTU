...
async def cmd_login(args):
    ...
    import subprocess
    # Invoke via the same Python interpreter to avoid PATH/entry-point issues
...
async def cmd_sources(args):
    ...
    nb_id = _resolve_nb_id(args.notebook_id) or _get_active()
    if not nb_id:
...
def _resolve_nb_id(value):
    """If value matches a library alias slug, return the real UUID. Otherwise return as-is.

    Lets users pass --notebook-id <alias> instead of remembering UUIDs.
    """
    if not value:
        return value
    lib = load_library()
    slug = value.lower().replace(" ", "-")
    if slug in lib.get("notebooks", {}):
        return lib["notebooks"][slug]["id"]
    return value
