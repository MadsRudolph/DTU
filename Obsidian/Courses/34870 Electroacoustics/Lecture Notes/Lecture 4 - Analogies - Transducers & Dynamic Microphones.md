...
> **b) Back volume for $f_0 = 1$ kHz.** $C_{MT} = 1/(\omega_0^2 M_{MT}) = 1/((2\pi\cdot1000)^2\cdot2.06\times10^{-4}) = 1.23\times10^{-4}$ m/N → $S_D^2/C_{AB} = 1/C_{MT} - 1/C_{MS} = 8150 - 4762 = 3390$ N/m → $C_{AB} = 2.567\times10^{-7}/3390 = 7.58\times10^{-11}$ m⁵/N → $V = C_{AB}\rho c^2 = \boxed{10.6\ \text{cm}^3}$. The sheet says 10.8 cm³. **Corrected 14-Sep after reading the official solutions:** the difference is not rounding but the *front air mass convention* — the official solution (and the 4B slide's "piston on a cylinder") uses the **piston-in-a-tube** value $M_{A1} = 0.6133\rho/(\pi a) = 18.1$ kg/m⁴, not the baffled $8\rho/(3\pi^2 a) = 25.1$ used above. With 18.1: $M_{MT} = 0.2047$ g, $C_{MT} = 1.238\times10^{-4}$, $S_D^2/C_{AB} = 3319$ N/m, $C_{AB} = 7.74\times10^{-11}$ → **10.8 cm³** exactly. Q = 0.127, BW = 7.88 kHz, $f_a$ = 125 Hz, $f_b$ = 8.01 kHz all follow. The Analogy Bench uses the sheet's convention by default with a toggle for the baffled value.
>
