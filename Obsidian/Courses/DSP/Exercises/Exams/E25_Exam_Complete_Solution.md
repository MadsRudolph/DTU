...
$$(z - \frac{1+i}{2})(z - \frac{1-i}{2}) = z^2 - z + \frac{1}{2}$$
...
**Expanding numerator:**
$$(z + 2)(z^2 - z + 0.5) = z^3 - z^2 + 0.5z + 2z^2 - 2z + 1$$
$$= z^3 + z^2 - 1.5z + 1$$

**Expanding denominator:**
$$z(z - 1/3)(z - 2/3) = z(z^2 - z + 2/9) = z^3 - z^2 + \frac{2z}{9}$$
...
**Reasoning:**
All three poles are strictly inside the unit circle (|p| < 1). For a causal system with transfer function H(z), this ensures:
...
Formula provided:
$$\mathcal{Z}\{a^n \sin(\omega_0 n) u[n]\} = \frac{az^{-1}\sin(\omega_0)}{1 - 2az^{-1}\cos(\omega_0) + a^2z^{-2}}, \quad |z| > |a|$$
...
$$2 \cdot \frac{\sqrt{2}}{2} \cdot \frac{\sqrt{2}}{2} = 2 \cdot \frac{2}{4} = 1$$
...
**Answer:** 
$$X_1(z) = \frac{0.5z^{-1}}{1 - z^{-1} + 0.5z^{-2}}, \quad |z| > 0.7071$$
...
**Simplify numerator:**
$$0.5z^{-1}(1 + z^{-1} - 1.5z^{-2} + z^{-3})$$
$$= 0.5z^{-1} + 0.5z^{-2} - 0.75z^{-3} + 0.5z^{-4}$$

**Expand denominator:**
$$(1 - z^{-1} + \frac{2}{9}z^{-2})(1 - z^{-1} + 0.5z^{-2})$$

Using FOIL:
$$= 1 - z^{-1} + 0.5z^{-2} - z^{-1} + z^{-2} - 0.5z^{-3} + \frac{2}{9}z^{-2} - \frac{2}{9}z^{-3} + \frac{1}{9}z^{-4}$$
...
**Any transfer function can be decomposed as:**
...
**All-pass structure:**
For each zero outside unit circle at z = z₀:
...
$$H_{ap}(z) = \frac{z^{-1} - z_1^*}{1 - z_1^* z^{-1}} = \frac{z^{-1} - (-2)}{1 - (-2)z^{-1}}$$

$$= \frac{z^{-1} + 2}{1 + 2z^{-1}}$$

Or equivalently:
$$H_{ap}(z) = \frac{1 + 2z^{-1}}{2 + z^{-1}}$$
...
For real z₀ = -2:
$$H_{ap}(z) = \frac{-(-2) + z^{-1}}{1 - (-2)z^{-1}} = \frac{2 + z^{-1}}{1 + 2z^{-1}}$$
...
Where K' is determined by requiring:
$$H(z) = H_{mp}(z) \cdot H_{ap}(z)$$
...
$$\Delta F = F_{pass} - F_{stop} = 1750 - 1250 = 500 \text{ Hz}$$

Normalized sharpness parameter:
$$F_{sharpness} = \frac{\Delta F}{F_s} = \frac{500}{5000} = 0.1$$
...
Where:
$$\Delta \omega = 2\pi \Delta \tilde{f} = 2\pi \times 0.1 = 0.2\pi \text{ rad/sample}$$
...
Let's try:
$$N_{taps} \approx \frac{20 \cdot 5000}{22 \cdot 500} = \frac{100000}{11000} \approx 9.09$$
...
Using Hamming formula:
$$M = \lceil \frac{3.3 \cdot F_s}{\Delta F} \rceil = \lceil \frac{3.3 \times 5000}{500} \rceil = \lceil 33 \rceil = 33$$

Wait, that gives Ntaps = 34, not 31...

Let me use more accurate formula:
$$M = \frac{(A_s - 8)}{2.285 \cdot \Delta \omega}$$
...
For Hamming with As = 40 dB:
$$N_{taps} \approx \frac{3.3 \cdot F_s}{\Delta F} + 1 = \frac{3.3 \times 5000}{500} + 1 = 33 + 1 = 34$$
