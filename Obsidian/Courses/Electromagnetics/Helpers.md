...
For a **uniform plane wave** in a (possibly lossy) medium with known intrinsic impedance $\eta$:

- Compute time-average Poynting vector magnitude $|\bar{S}|$
- Compute power through a finite area for oblique incidence

Typical pattern:

- Given $|E_0|$ and $\eta$ → $\bar{S} = |E_0|^2 / (2|\eta|)$
- Then project with $\cos\varphi$ on a surface of area $A$
...
- `A` — Area [m²]
- `phi` — Angle between **Poynting vector** and surface normal [rad]
...
- `S_mag` — $|\bar{S}|$ [W/m²]
- `P_incident` — Power hitting surface [W]

### Example

```matlab
eta0 = 120*pi;
pw = poynting_pw(10, eta0, 0.01, 0);   % normal incidence on 10 cm²
```

### MATLAB — `poynting_pw.m`

> [!code]- MATLAB — poynting_pw.m
> 
> ```matlab
...
## 7. `coulomb_pair` — Force between two point charges

### Purpose

Implement **Coulomb's law** in vector form for two point charges:

- Accept positions in meters
- Return vector force on $q_1$ due to $q_2$ and on $q_2$ due to $q_1$

Matches the style of the early Electrostatics exercises.
...
- `F12` — Force on $q_1$ due to $q_2$ (vector) [N]
- `F21` — Force on $q_2$ due to $q_1$ (vector) [N]
...
### MATLAB — `coulomb_pair.m`

> [!code]- MATLAB — coulomb_pair.m
> 
> ```matlab
...
## 8. `B_inf_wire` — B-field of an infinitely long wire

### Purpose

Compute the magnitude of the **magnetic flux density** $B$ around an infinitely long straight current-carrying wire:

- Use Biot–Savart / Ampère's law result:

$$B = \frac{\mu I}{2\pi r}$$

Useful for basic Magnetostatics I problems.
...
- `B` — Magnitude of B-field [T]

### Example

```matlab
B = B_inf_wire(5, 0.02);      % 5 A, 2 cm from wire in free space
```

### MATLAB — `B_inf_wire.m`

> [!code]- MATLAB — B_inf_wire.m
> 
> ```matlab
