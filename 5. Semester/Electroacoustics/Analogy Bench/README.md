...
- **Live (LAN):** http://192.168.50.220 — Proxmox CT 116 `study`, nginx.
...
- **Deploy:** `./deploy.sh` (build → scp to the Proxmox host → `pct push` → CT 116).
- **Local preview:** `.claude/launch.json` config `analogy-bench` (python http.server on 8642).

No public hostname yet: that needs a cloudflared tunnel + DNS route, which needs
a one-time `cloudflared tunnel login` — see the homelab skill.
