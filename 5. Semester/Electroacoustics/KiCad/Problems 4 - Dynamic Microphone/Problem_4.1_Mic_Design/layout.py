#!/usr/bin/env python3
"""Problems 4, problem 1 - the designed dynamic microphone (1b: V_AB = 10.8 cm^3
for f_0 = 1 kHz; 1c: R_AF = 3.56e7 for M = 1 mV/Pa). Same three-domain circuit as
Problem_4B, only the back volume and felt resistance differ.
...
NAME = "Problem_4.1_Mic_Design"

rho, c = 1.18, 344.0
MMD, RMS, CMS = 0.2e-3, 1.0, 0.21e-3
a = 0.0254 / 2; SD = np.pi * a**2
Bl, RE, RL = 20.0, 200.0, 47e3
V, RAF = 10.8e-6, 3.56e7
MA1 = 8 * rho / (3 * np.pi**2 * a)
CAB = V / (rho * c**2)
AC_CMD = ".ac dec 200 10 100k"

sh = Sheet(paper="A4", title="Problems 4 (1) - designed dynamic microphone, f0 = 1 kHz, M = 1 mV/Pa", project=NAME)
...
# ---------- notes ----------
sh.note((G(14), G(6)), "Problems 4 / 1 - the designed microphone: V_AB = 10.8 cm^3 (1b), R_AF = 3.56e7 (1c), 1 Pa in -> V(out)", size=1.6)
sh.note((G(14), G(10)), f"Acoustic (p = V, U = I): V1 = p_i = 1 Pa, L1 = M_A1 = {MA1:.3g} kg/m^4 (baffled piston, a = 12.7 mm), R1 = R_AF = {RAF:.3g}, C1 = C_AB = {CAB:.3g} (V = 10.8 cm^3, from 1b), R3 = 1 TOhm DC leak (>> 1/wC_AB even at 10 Hz)", size=1.2)
...
sh.note((G(14), G(19)), f"Electrical: E1 = Bl*u (Bl = {Bl:g} Tm), R5 = R_E = {RE:g}, R6 = R_L = {RL:g}. Expect f0 = 1 kHz, M = 1 mV/Pa = -60 dB re 1 V/Pa, band 125 Hz - 8 kHz (1d)", size=1.2)
