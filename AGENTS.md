# AGENTS.md — repo handoff

Auto-loaded by Codex, travels with `git pull`. Records **where to pick up** so a fresh session on any PC continues seamlessly.
...
- **34840 Lecture 4 (22-Sep-2026)** — notes on impedance, intensity and power, all Week 4 problems plus exam examples solved, slides converted to PDF, and explanatory MATLAB Live Script/source/HTML in `5. Semester/Acoustics and Noise Control/Matlab/Week4_Impedance_Intensity_Power.*`. Sound Bench covers lectures 1–4 with five Lecture 4 benches and a quiz.
- **34870 Lab A (quiz deadline Sun 20-Sep)** — fully worked, 10-Sep-2026. LaTeX report `5. Semester/Electroacoustics/Labs/Lab A/report/LabA_report.tex` (PDF carried by Syncthing, copy in the vault `Labs/`), Obsidian runthrough `Obsidian/Courses/34870 Electroacoustics/Labs/Lab A - Runthrough.md` (quiz-ready answers per sub-question + LTspice translation), KiCad/ngspice projects under `Labs/Lab A/KiCad/PartN_*` regenerated by `Labs/Lab A/sim/partN.py`. **Still to do by hand:** rebuild the circuits in LTspice for the quiz screenshots (the quiz is LTspice-based) and submit in DTU Learn.
- **34870 Lecture 0–4 notes** in `Lecture Notes/`, each with KiCad/ngspice companion sims for the in-class problems (`5. Semester/Electroacoustics/KiCad/<Lecture N | Problems 4>/Problem_*`).
...
### Commits
- **NEVER** add `Co-Authored-By: Codex` or any mention of Codex/AI. Commit messages read like a developer wrote them.
...
### NotebookLM
`python ~/.Codex/skills/notebooklm/scripts/nlm.py ask "..." --notebook-id <id>` (`nlm.bat` is the old Windows wrapper) — shortnames `lcd1`, `dsp`; 62711 = `eb1f49b9-61a5-4494-8a3e-9821f8514324`, DSP = `5bd40a62-b09c-406d-b854-2ed2be6d894c`. Re-`login` if `auth-status` says NOT AUTHENTICATED; intermittent read-timeouts, retry.
...
- [`project_62711_exam_prep_hub`](.Codex/projects/.../memory/) — index of where the exam-prep folder lives, decisions locked in
- [`reference_notebooklm_ids`](.Codex/projects/.../memory/) — 62711 notebook UUID = `eb1f49b9-61a5-4494-8a3e-9821f8514324`
- [`feedback_use_mermaid`](.Codex/projects/.../memory/) — prefer mermaid diagrams over ASCII art in Obsidian notes
...
- **NotebookLM:** `python ~/.Codex/skills/notebooklm/scripts/nlm.py ask "..." --notebook-id eb1f49b9-61a5-4494-8a3e-9821f8514324` for fact-checking against ingested 62711 course material. Re-`login` if `auth-status` says NOT AUTHENTICATED; intermittent read-timeouts — retry.
...
- **Commits:** NEVER add `Co-Authored-By: Codex` or any AI mention. Commit messages read like a developer wrote them.
